 ...
 */
const _seen_wf = new Set<string>();
export const WORKFLOWS = _RAW_WORKFLOWS.filter((item) => {
  if (!item || !item.id || _seen_wf.has(item.id)) return false;
  _seen_wf.add(item.id);
  return true;
});
