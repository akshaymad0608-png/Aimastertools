...
/**
 * The raw array above no longer carries duplicates: the 195 repeated ids and
 * the 30 records curated twice under different ids have been removed, so
 * _MOCK_TOOLS is already the 654 tools this file exports.
 *
 * Both passes stay as guards. Tools are appended here in batches, and a batch
 * that reintroduces an id would otherwise mean React key collisions, the same
 * card twice in one list, inflated category counts, and duplicate-content
 * signals pointing at /tool/:id routes that can only resolve to one record.
 * Against the data as it stands they are no-ops.
 *
 * First occurrence wins, so the earliest curated entry survives.
 */
...
/**
 * Second guard: collapse entries that share a normalized name. Id-dedup alone
 * used to leave 30 products showing twice, because the same tool was curated
 * under two different ids — "midjourney-v7" and "midjourney", "lovable-code"
 * and "lovable". Two cards, two /tool routes, one real product.
 *
 * Keeps the shortest (most canonical) id — also the URL most likely to be
 * indexed already, so a ranking page is never orphaned — and prefers a record
 * carrying a longDescription when ids tie. Order is preserved.
 *
 * prerender.mjs runs this same pair of passes over the raw array, so the two
 * must agree on which record wins; keep them in step if either changes.
 */
